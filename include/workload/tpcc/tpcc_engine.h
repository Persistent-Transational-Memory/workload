...
#pragma once
#ifndef WORKLOAD_TPCC_TPCC_DB_H
#define WORKLOAD_TPCC_TPCC_DB_H
	...
	inline namespace tpcc {
		...
		public:
			...
			bool payment(int32_t warehouse_id, int32_t district_id, int32_t c_warehouse_id,
				...
				Customer *customer_ptr = update_customer(c_warehouse_id, c_district_id, customer_id);
			...
			bool payment(int32_t warehouse_id, int32_t district_id, int32_t c_warehouse_id,
				...
				Customer *customer_ptr = update_customer(temp_customer.c_w_id, temp_customer.c_d_id, temp_customer.c_id);
				if (customer_ptr == nullptr) [[unlikely]] { return false; }
				Customer &customer = *customer_ptr;
		...
		private:
			...
			bool find_and_validate_items(const auto item_iter_begin, const auto item_iter_end,
				...
				for (auto iter = item_iter_begin; iter != item_iter_end; ++iter, ++output_iter) {
					...
					std::memcpy(std::addressof(*output_iter), item_ptr, sizeof(Item));
				}
				return true;
			}
